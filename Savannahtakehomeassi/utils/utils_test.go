...
func TestCompareAWSInstanceWithTerraform(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectDrift:  false,
			expectError:  false,
			expectedMsgs: []string{"Block Device Mapping drift detected: AWS={/dev1/xvda vol-2}, Terraform=map[/dev/xvda:{false /dev/xvda false vol-1 0 } vol-1:{false /dev/xvda false vol-1 0 }]"},
		},
		{
		...
		},
		{
			...
			expectDrift:  false,
			expectError:  false,
			expectedMsgs: []string{"PrivateDnsName drift detected: AWS=ip-10-0-0-1.ec2.internal-test, Terraform=ip-10-0-0-1.ec2.internal"},
		},
		{
